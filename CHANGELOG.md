...
- **PoC Revision 5 remediation** (user-reported bugs in v0.2.3):
  ...
  - Showcase gained a generic `linkNote`; Facebook stays area-level with its login-gate note. **Cazoo upgraded (same day):** product owner supplied a real single-car PDP (`cars-for-sale/79613297`, a Ford Fiesta) — now linked directly; card figures remain labelled representative-of-model because Cazoo's bot-wall prevents machine-verifying the page numbers.
