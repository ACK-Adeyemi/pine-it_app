...
- **Facets stay data-driven (ADR-0002).** The "Condition & running" section exposes **Running status as a normal multi-select** (*Running · Spares & Repairs · Non Starter*) plus the relocated **Condition** multi-select; each option reports its own honest membership count. The default selection `{Running}` reproduces the "clean hero feed" behaviour with zero special-casing in the engine — chips/badges/empty-hints treat it as non-active so it never reads as a chosen filter. `For parts` has **no standalone option**: every For-parts car is by definition a Spares & Repairs car, so one option governs (sellers still record For-parts via the post form). Badges (amber/red) appear on cards and the detail modal, and the detail modal renders the full `attrs` spec sheet.
- *(v0.3.2 restructure)* An earlier Rev 6 cut implemented the two non-runner surfaces as special "reveal toggles" with a hidden gate inside `matches()`; that produced misleading counts (`For parts: 0`, inflated 696/691 toggle counts). The multi-select restructure removed the gate entirely and fixed all counts.
