...
2. **Deep links opened category/search pages (PLPs), not advert pages (PDPs).** Every homeUrl now points at a **verified live single-ad page**, captured and cross-checked against the card's facts at build time:
   ...
   - Cazoo → `https://www.cazoo.co.uk/cars-for-sale/79613297/` — a **real single-car PDP supplied by the product owner** (confirmed a Ford Fiesta). Cazoo's bot-wall still blocks machine verification of that page's figures, so the card's year/price/mileage stay explicitly labelled *representative of this model tier* on-card and in the data description; make/model and the deep link are real.
