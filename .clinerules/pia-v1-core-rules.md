...
### PoC Revision 7
  ...
  - **Implemented (v0.4.0, 2026-08-27)** — see ADR-0007. All ad content/visibility lives in the external **`docs/ad-config.json`** (built-in defaults cover `file://`; admin edits override per-device until published via the console's **Download ad-config.json**). Placements: **fixed banner under the nav**, full-width **ad banner row every 9 cards**, **embedded sponsored card every 10 cards** in a car-ad slot — all labelled, paid links `rel="noopener sponsored"`. Sponsored has a **3-way mode** (per stakeholder request): *paid* third-party items, *default* (slot replaced with Pine It's own feature promos — post-an-ad, filter engine, one-feed-every-platform), or *off*; an empty paid list falls back to the default promos, never a blank slot. The **demo admin console** (header **Admin** button, no login — stakeholder decision) manages banners & sponsored and adds **ads moderation** (hide/restore/delete posted ads; hides are reversible render-time filters via `visibleListings()`, so counts/facets stay honest) and **accounts moderation** (accounts = posters via an optional post-form display name + lead requesters via name/email; block/unblock suppresses their content) — state in `localStorage['pineit_moderation']`. Production path documented in `PoC-PRODUCTION-READINESS.md`: a **separate auth-protected console URL location** with roles, server-side moderation flags/audit trails and a real ad server/CMS.
